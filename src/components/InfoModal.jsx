...
import { SANS, MONO, SERIF, INK, MUTE, LINE } from "../theme";
...
// principle described in the spec.
export function InfoIcon({ type, entity, data }) {
  return (
    <TraceButton type={type} entity={entity} data={data} />
  );
}

function TraceButton({ type, entity, data }) {
    ...
    <>
      ...
      {open && (
        <Modal onClose={() => setOpen(false)} width={380}>
          ...
          <TraceChain type={type} entity={entity} data={data} />
...
export function TraceChain({ type, entity, data }) {
